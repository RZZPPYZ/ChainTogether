...
# Group chat -- multi-agent conversations with @-mention routing (groups.md).
...
class GroupSendRequest(BaseModel):
    """Send a message to a group. The backend parses @-mentions to decide
    which agent(s) to wake. With no @-mention the message is stored as a
    plain user message but no agent is invoked."""
