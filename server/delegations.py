...
from .session_chain import ChainWalkError, walk_parent_chain
...
class DelegationManager:
    ...
    async def _check_chain(self, parent, *, target_agent_id: str) -> None:
        ...
        returns None for a non-null ancestor id, the shared walker
        falls back to the DB (``include_archived=True``) so that a
        legitimately archived delegation parent still contributes its
        agent_id / origin to the walk. This matters after the round-2
        fix that archives delegation children once their terminal turn
        is delivered: without the DB fallback, an unarchived child
        whose parent remains archived would over-reject every
        subsequent ``ask_agent``.

        Only ``origin == 'delegation'`` rows contribute to the depth
        count; ``origin == 'group_reply'`` rows (group-A2A children)
        are skipped here so a delegation hanging off a group_reply
        chain isn't double-penalised by the group's own depth cap.
        Group-A2A has its own cap in
        ``group_manager._check_a2a_chain``.
        """
        assert self.session_mgr is not None
        assert self.db is not None
        try:
            walk = await walk_parent_chain(
                self.session_mgr, self.db, parent.id,
            )
        except ChainWalkError as exc:
            # Translate the shared walker's fail-closed conditions
            # into the public DelegationError(409) shape callers
            # already handle. Same statuses + messages as the inline
            # implementation did before extraction.
            if exc.kind == "session_id_cycle":
                raise DelegationError(
                    "Caller chain has a session-id cycle "
                    "(corrupted parent_session_id pointers)",
                    status_code=409,
                ) from exc
            if exc.kind == "broken_pointer":
                raise DelegationError(
            ...
            # exceeded_safety_cap and any future fail-closed kind.
            raise DelegationError(
                f"Caller chain exceeds the safety hop cap; refuse "
                f"as a fail-closed guard against pointer corruption",
                status_code=409,
            ) from exc

        if target_agent_id in walk.agent_ids:
            raise DelegationError(
        ...
        if walk.delegation_hops + 1 > DEPTH_CAP:
            raise DelegationError(
