...
CREATE TABLE IF NOT EXISTS messages (
    ...
    agent_id TEXT,                          -- which agent produced this row (group chats);
                                            -- NULL for user msgs and normal single-agent sessions
...
-- Multi-agent group chat (groups.md). A group owns ONE backing session
-- (origin='group', owned by the system agent) that stores every message.
-- @-mention routing uses one long-lived member session per (group, agent).
...
-- group + member-agent join. A group needs >= 2 members. Cascading on
-- group delete clears the membership; cascading on agent delete removes
-- the agent from every group it belonged to.
...
-- (group × agent) long-lived member sessions for group chat. Created lazily
-- on the first @-mention of a member agent, reused on every subsequent @
-- (within the same group) so the agent keeps its private --resume JSONL
-- transcript across turns. FK CASCADE on group delete clears this table
-- alongside groups + group_members; FK CASCADE on session delete is the
-- signal that the (group × agent) session was itself hard-deleted (via
-- delete_group / remove_member) — group_manager always deletes the
-- session row first, so this row vanishes atomically.
...
-- One durable routing lifecycle per user message that wakes group agents.
-- Runtime queues remain in memory, while this row provides cancellation,
-- custody visibility and crash reconciliation across server restarts.
CREATE TABLE IF NOT EXISTS group_invocations (
    id TEXT PRIMARY KEY,
    group_id TEXT NOT NULL,
    root_content TEXT NOT NULL,
    status TEXT NOT NULL,                 -- running|held|completed|blocked|failed|cancelled
    custody_state TEXT NOT NULL,          -- new|active|held|blocked|void|dead|resolved|cancelled
...
CREATE INDEX IF NOT EXISTS idx_group_invocations_group
...
class Database:
    ...
    async def _migrate_schedule_recurrence(self) -> None:
        ...
        # messages.agent_id (group chat) -- additive on legacy DBs. Fresh
        # DBs already have the column from _SCHEMA; guarded so re-running
        # no-ops.
        if not await self._has_column("messages", "agent_id"):
            await self._conn.execute(
                "ALTER TABLE messages ADD COLUMN agent_id TEXT"
            )
        if not await self._has_column("groups", "default_agent_id"):
            await self._conn.execute(
    ...
    async def get_group(self, group_id: str) -> dict[str, Any] | None:
        ...
        if row is None:
            return None
        members = await self._group_members(group_id)
        return {
            "id": row[0],
            "name": row[1],
            "session_id": row[2],
    ...
    async def list_groups(self) -> list[dict[str, Any]]:
        ...
        for row in rows:
            members = await self._group_members(row[0])
            out.append({
                "id": row[0],
                "name": row[1],
                "session_id": row[2],
    ...
    async def update_group(
        self, group_id: str, **fields: Any,
    ) -> dict[str, Any] | None:
        allowed = {"name", "default_agent_id"}
    ...
    async def add_group_member(
        self, group_id: str, agent_id: str, joined_at: str
    ) -> None:
        await self._ensure_connected()
        await self._conn.execute(
            "INSERT OR IGNORE INTO group_members "
    ...
    async def remove_group_member(self, group_id: str, agent_id: str) -> None:
        ...
        await self._conn.execute(
            "UPDATE groups SET default_agent_id = NULL "
            "WHERE id = ? AND default_agent_id = ?",
            (group_id, agent_id),
        )
        await self._conn.commit()

    async def _group_members(self, group_id: str) -> list[str]:
        cursor = await self._conn.execute(
    ...
    async def upsert_group_agent_session(
        self, group_id: str, agent_id: str,
        session_id: str, created_at: str,
    ) -> None:
        """Record the (group × agent) reuse session id. INSERT OR REPLACE so
        the first @-mention creates it; remove_member's hard-delete-then-
        re-@ path will INSERT a fresh row when the agent rejoins."""
        await self._ensure_connected()
        await self._conn.execute(
            "INSERT OR REPLACE INTO group_agent_sessions "
            "(group_id, agent_id, session_id, created_at) "
            "VALUES (?, ?, ?, ?)",
            (group_id, agent_id, session_id, created_at),
        )
        await self._conn.commit()

    async def get_group_agent_session(
        self, group_id: str, agent_id: str,
    ) -> str | None:
        """Return the session_id for (group × agent), or None if the agent
        hasn't been @-mentioned in this group since the row was cleared."""
        await self._ensure_connected()
        cursor = await self._conn.execute(
            "SELECT session_id FROM group_agent_sessions "
    ...
    async def list_group_agent_sessions(
        ...
        """All (agent_id, session_id) pairs for a group. Used by delete_group
        to cascade-hard-delete member sessions alongside the backing session."""
        await self._ensure_connected()
        cursor = await self._conn.execute(
            "SELECT agent_id, session_id FROM group_agent_sessions "
            "WHERE group_id = ?",
            (group_id,),
        )
        rows = await cursor.fetchall()
        return [{"agent_id": r[0], "session_id": r[1]} for r in rows]

    async def delete_group_agent_session(
        self, group_id: str, agent_id: str,
    ) -> None:
        """Clear the reuse pointer for (group × agent). Called after the
        underlying session row is hard-deleted (FK CASCADE also clears this
        row, but explicit deletion keeps the contract obvious for callers)."""
        await self._ensure_connected()
        await self._conn.execute(
    ...
    # ------------------------------------------------------------------
    # Durable group invocation lifecycle
    # ------------------------------------------------------------------

    async def create_group_invocation(
        self,
        invocation_id: str,
        group_id: str,
        root_content: str,
    ...
    async def update_group_invocation(
        self, invocation_id: str, **fields: Any,
    ) -> None:
        allowed = {
            "status", "custody_state", "current_agent_id", "depth",
            "held_until", "hold_reason", "error", "updated_at",
            "completed_at",
        }
        ...
        await self._conn.execute(
            f"UPDATE group_invocations SET {assignments} WHERE id = ?",
            (*updates.values(), invocation_id),
        )
        await self._conn.commit()

    async def get_group_invocation(
        ...
        row = await cursor.fetchone()
        return self._group_invocation_row(row) if row else None

    async def list_group_invocations(
        ...
        if active_only:
            sql += " AND status IN ('running', 'held')"
        sql += " ORDER BY created_at DESC"
        cursor = await self._conn.execute(sql, params)
        return [self._group_invocation_row(row) for row in await cursor.fetchall()]

    async def list_recoverable_group_invocations(self) -> list[dict[str, Any]]:
        await self._ensure_connected()
        cursor = await self._conn.execute(
            "SELECT id, group_id, root_content, status, custody_state, "
            "current_agent_id, depth, held_until, hold_reason, error, "
    ...
    @staticmethod
    def _group_invocation_row(row: Any) -> dict[str, Any]:
        return {
            "id": row[0],
            "group_id": row[1],
            "root_content": row[2],
    ...
    async def append_feature_run_event(
        ...
        *,
        ...
        await self._conn.execute(
            ...
            " reason, evidence_refs, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                run_id,
                from_stage,
                to_stage,
    ...
    async def list_feature_run_events(
        ...
        cursor = await self._conn.execute(
            ...
            "actor_agent_id, reason, evidence_refs, created_at "
    ...
    async def append_message(
        ...
        await self._conn.execute(
            ...
            "(session_id, seq, role, type, content, tool_name, tool_input, "
            "tool_use_id, is_error, session_id_ref, cost, attachments, "
            "git_head, git_status_clean, agent_id) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                session_id,
                seq,
