...
class SessionManager:
    ...
    # active list. 'schedule' and 'group_reply' are auto-archived from
    # idle. delegation children must NOT be, because a delegation parent
    ...
    # group_reply sessions (spawned by @-mention routing) are safe to
    # archive immediately because their reply is injected into the
    # parent group session before the child goes idle.
    _AUTO_ARCHIVE_ORIGINS = ("schedule", "group_reply")
    ...
    _AUTO_ARCHIVE_ELIGIBLE = ("schedule", "delegation", "group_reply")
    ...
    async def inject_message(
        ...
        *,
        ...
        """Persist + broadcast a text message WITHOUT spawning a backend turn.

        Group chat uses this for both user messages and agent replies: the
        group session stores the full transcript, but the owning system
        agent never runs a turn. @-mention routing spawns separate child
        sessions (via start_message) for the actual agent work.

        `role` is a MessageRole value string ('user' | 'assistant'). The
        broadcast event is shaped like a normal user_message/assistant_text
        event so the existing WS handler + frontend store pick it up with
        no special-casing. `agent_id` (when set) is persisted on the row so
        group context formatting can attribute past replies.
        """
        session = self.sessions.get(session_id)
        if session is None:
            raise ValueError(f"Session {session_id} not found")
        msg = MessageContent(role=MessageRole(role), type="text", content=content)
        seq = await self._persist_message(session, msg, agent_id=agent_id)
        event: dict[str, Any] = {
            "type": "user_message" if role == "user" else "assistant_text",
    ...
    def _run_config(
        ...
        project_prompts = project_system_prompts(
            ...
            agent,
            session.backend,
            group_member=session.origin == "group_member",
        )
        if project_prompts:
            system_prompt = "\n\n".join(
