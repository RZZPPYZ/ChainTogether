...
function handleWsMessage(data: Record<string, unknown>) {
  ...
  switch (type) {
    ...
    case "user_message":
      ...
      // When a [agent-reply:Name] message arrives in a group session,
      // clear the streaming reply for that agent — the committed
    ...
    case "group_typing": {
      ...
      if (sessionId && agentName) {
        getState().addGroupTypingAgent(sessionId, agentName);
      }
      break;
    }

    case "group_typing_done": {
      const agentName = data.agent_name as string;
      if (sessionId && agentName) {
        getState().removeGroupTypingAgent(sessionId, agentName);
      }
      break;
    }

    case "group_agent_text": {
      const agentName = data.agent_name as string;
    ...
    case "group_invocation": {
      ...
      if (invocation && typeof invocation === "object") {
        getState().upsertGroupInvocation(
          invocation as GroupInvocation,
        );
      }
      break;
    }
